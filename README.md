...
Отредактируйте публичный файл [`gift_descriptions.json`](gift_descriptions.json). Мишки расположены от нового к старому и подписаны точным временем выхода по Екатеринбургу (`UTC+5`), поэтому ориентироваться по ID не требуется. Заполнять нужно только поле `description`:
...
Описание длиной до 120 символов появляется на кнопке выбора, экране ввода текста, подтверждении и в финальном чеке. Пробельные символы автоматически нормализуются, пустые значения игнорируются. Файл перечитывается при каждом `/gift`, поэтому перезапуск после редактирования не нужен.
...
Поля `released_at`, `name` и `gift_id` менять не нужно. JSON специально отделён от секретного `config.py`, поэтому заполненные описания можно безопасно публиковать в репозитории для всех пользователей.
