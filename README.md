...
Отредактируйте публичный файл [`gift_descriptions.json`](gift_descriptions.json). Ключ — технический ID подарка, значение — короткое описание его внешнего вида:

```json
{
  "6046178578163303744": "в тёмных очках и с красным шарфом"
}
```
...
Этот JSON специально отделён от секретного `config.py`, поэтому заполненные описания можно безопасно публиковать в репозитории для всех пользователей.
